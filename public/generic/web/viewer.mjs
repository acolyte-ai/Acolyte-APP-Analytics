...
window.PDFViewerAPI = {
  search: function (query, options = {}) {
    ...
    setTimeout(() => {
      if (window.parent) {
        const info = this.getSearchMatchInfo();
  ...
  clearSearch: function () {
    ...
    if (findController) {
      ...
      findController._state = null;
      findController._rawQuery = "";
      findController._normalizedQuery = "";
      findController._matchesCountTotal = 0;
    }
    if (window.parent) {
      window.parent.postMessage(
        {
  ...
  findNext: function () {
    const { findController } = PDFViewerApplication;
    if (!findController?.state) {
      console.warn("No active search to navigate");
      return;
    }
    PDFViewerApplication.eventBus.dispatch("find", {
      source: window,
      type: "again",
      findPrevious: false,
      highlightAll: findController.highlightMatches,
      query: findController._rawQuery,
      phraseSearch: findController.state.phraseSearch,
      caseSensitive: findController.state.caseSensitive,
      entireWord: findController.state.entireWord,
      matchDiacritics: findController.state.matchDiacritics,
    });
    setTimeout(() => {
      if (window.parent) {
        const info = this.getSearchMatchInfo();
  ...
  getSearchMatchInfo: function () {
    const { findController } = PDFViewerApplication;
    console.log("this is from find", findController);
    if (!findController || !findController.state) {
      return {
        current: 0,
        total: 0,
        rawQuery: "",
    ...
    if (findController._selected) {
      const { pageIdx, matchIdx } = findController._selected;
      if (pageIdx !== -1) {
        for (let i = 0; i < pageIdx; i++) {
          current += findController._pageMatches[i]?.length || 0;
        }
        if (matchIdx !== -1) {
          current += matchIdx + 1;
        }
  ...
  setAnnotationEditorMode: function (mode) {
    if (!PDFViewerApplication.pdfViewer) {
      console.warn("PDF Viewer not initialized");
      return 0;
    }
    console.log("[PDFViewerAPI] Setting annotation editor mode:", mode);
    console.log("[PDFViewerAPI] Current viewer state:", {
      isInitialized: !!PDFViewerApplication.pdfViewer,
      documentLoaded: !!PDFViewerApplication.pdfDocument,
      currentPage: PDFViewerApplication.page,
    });
    const AnnotationEditorType = PDFViewerApplication.pdfViewer
      .annotationEditorUIManager?.editorTypes || {
      DISABLE: 0,
      HIGHLIGHT: 9,
      FREETEXT: 3,
      INK: 15,
    ...
    console.log(
      "[PDFViewerAPI] Available annotation editor types:",
      AnnotationEditorType
    );
    const modeMap = {
      highlight: AnnotationEditorType.HIGHLIGHT || 9,
      freetext: AnnotationEditorType.FREETEXT || 3,
      ink: AnnotationEditorType.INK || 15,
      stamp: AnnotationEditorType.STAMP || 13,
      signature: AnnotationEditorType.SIGNATURE || 12,
      none: AnnotationEditorType.DISABLE || 0,
    };
    console.log("[PDFViewerAPI] Mode mapping:", modeMap);
    const editorMode =
      typeof mode === "string" ? modeMap[mode.toLowerCase()] || 0 : mode || 0;
    console.log("[PDFViewerAPI] Selected editor mode:", editorMode);
    try {
      console.log(
        "[PDFViewerAPI] Dispatching switchannotationeditormode event with:",
        {
          mode: editorMode,
        }
      );
      ...
      console.log("[PDFViewerAPI] Event dispatched successfully");
    } catch (error) {
      console.error(
        "[PDFViewerAPI] Error dispatching annotation mode event:",
        error
      );
    }
    setTimeout(() => {
      const currentMode = this.getAnnotationEditorMode();
      console.log("[PDFViewerAPI] Mode after switch attempt:", currentMode);
    }, 100);
    if (window.parent) {
      window.parent.postMessage(
        {
          type: "annotationModeChange",
          data: {
            mode: editorMode,
          },
        },
  ...
  getAnnotationEditorMode: function () {
    if (!PDFViewerApplication.pdfViewer) {
      console.log(
        "[PDFViewerAPI] Cannot get annotation mode: viewer not initialized"
      );
      return 0;
    }
    const mode = PDFViewerApplication.pdfViewer.annotationEditorMode;
    console.log(
      "[PDFViewerAPI] Current annotation editor mode:",
      mode,
      "type:",
      typeof mode
    );
    console.log("[PDFViewerAPI] Annotation editor available:", {
      hasUIManager: !!PDFViewerApplication.pdfViewer.annotationEditorUIManager,
      editorTypes:
        PDFViewerApplication.pdfViewer.annotationEditorUIManager?.editorTypes,
    });
    return typeof mode === "number" ? mode : 0;
  },
  toggleHighlight: function (enable) {
    console.log("[PDFViewerAPI] Toggle highlight called with enable:", enable);
    const currentMode = this.getAnnotationEditorMode();
    console.log(
      "[PDFViewerAPI] Current mode before toggling highlight:",
      currentMode
    );
    const highlightMode = 9;
    const noneMode = 0;
    let newMode;
    if (enable === true) {
      console.log("[PDFViewerAPI] Explicitly enabling highlight mode");
      newMode = highlightMode;
    } else if (enable === false) {
      console.log("[PDFViewerAPI] Explicitly disabling highlight mode");
      newMode = noneMode;
    } else {
      console.log(
        "[PDFViewerAPI] Toggling highlight mode, current:",
        currentMode
      );
      newMode = currentMode === highlightMode ? noneMode : highlightMode;
      console.log("[PDFViewerAPI] New mode after toggle:", newMode);
    }
    const result = this.setAnnotationEditorMode(newMode);
    console.log("[PDFViewerAPI] Result of toggling highlight:", result);
    return result;
  },
  toggleFreeText: function (enable) {
    const currentMode = this.getAnnotationEditorMode();
    const freeTextMode = 3;
    const noneMode = 0;
    let newMode;
    if (enable === true) {
      newMode = freeTextMode;
    } else if (enable === false) {
      newMode = noneMode;
    } else {
      newMode = currentMode === freeTextMode ? noneMode : freeTextMode;
    }
    return this.setAnnotationEditorMode(newMode);
  },
  toggleInk: function (enable) {
    const currentMode = this.getAnnotationEditorMode();
    const inkMode = 15;
    const noneMode = 0;
    let newMode;
    if (enable === true) {
      ...
      newMode = currentMode === inkMode ? noneMode : inkMode;
    }
    return this.setAnnotationEditorMode(newMode);
  },
  toggleStamp: function (enable) {
    const currentMode = this.getAnnotationEditorMode();
    const stampMode = 13;
    const noneMode = 0;
    let newMode;
    if (enable === true) {
      ...
      newMode = currentMode === stampMode ? noneMode : stampMode;
    }
    return this.setAnnotationEditorMode(newMode);
  },
  toggleSignature: function (enable) {
    const currentMode = this.getAnnotationEditorMode();
    const signatureMode = 12;
    const noneMode = 0;
    let newMode;
    if (enable === true) {
      ...
      newMode = currentMode === signatureMode ? noneMode : signatureMode;
    }
    return this.setAnnotationEditorMode(newMode);
  },
  toggleSidebar: function (enable) {
    if (!PDFViewerApplication.pdfSidebar) {
      console.warn("PDF Sidebar not initialized");
      return;
    }
    if (enable !== undefined) {
      if (enable) {
    ...
    if (window.parent) {
      const isOpen = !PDFViewerApplication.pdfSidebar.isOpen;
      window.parent.postMessage(
        {
          type: "sidebarChange",
          data: {
  ...
  setSidebarView: function (view) {
    ...
    switch (String(view).toLowerCase()) {
      case "thumbnails":
        sidebarView = SidebarView.THUMBS;
        break;
      case "outline":
      case "bookmarks":
        sidebarView = SidebarView.OUTLINE;
        break;
      case "attachments":
        sidebarView = SidebarView.ATTACHMENTS;
        break;
      case "layers":
        sidebarView = SidebarView.LAYERS;
        break;
      default:
        console.error("Invalid sidebar view:", view);
        return;
    }
    PDFViewerApplication.pdfSidebar.switchView(sidebarView, true);
    if (window.parent) {
      window.parent.postMessage(
        {
          type: "sidebarViewChange",
          data: {
  ...
  getSidebarInfo: function () {
    ...
    const isOpen = PDFViewerApplication.pdfSidebar.isOpen;
    const viewNumber = PDFViewerApplication.pdfSidebar.visibleView;
    let view = "none";
    switch (viewNumber) {
      case 1:
        view = "thumbnails";
  ...
  goToPage: function (pageNumber) {
    ...
    if (
      isNaN(pageNumber) ||
      pageNumber < 1 ||
      pageNumber > PDFViewerApplication.pagesCount
    ) {
  ...
  getCurrentPage: function () {
    ...
    return {
      pageNumber: PDFViewerApplication.page || 0,
      pageLabel: PDFViewerApplication.pdfViewer.currentPageLabel || "",
      totalPages: PDFViewerApplication.pagesCount || 0,
    };
  },
  zoomIn: function () {
    ...
    setTimeout(() => {
      if (window.parent) {
        const zoomInfo = this.getCurrentZoom();
        window.parent.postMessage(
          {
            type: "zoomChange",
            data: zoomInfo,
  ...
  getCurrentZoom: function () {
    ...
    return {
      ...
      scaleValue: PDFViewerApplication.pdfViewer.currentScaleValue || "100%",
    };
  },
  // Adding new function to PDFViewerAPI
  setHalfWidth: function () {
    if (!PDFViewerApplication.pdfViewer) {
      console.warn("PDF Viewer not initialized");
      return false;
    }
    ...
    try {
      ...
      // Calculate half of the current scale
      const halfScale = currentScale * 0.5;

      // Set the new scale
      PDFViewerApplication.pdfViewer.currentScale = halfScale;
      ...
      console.error("Error setting half width:", error);
      return false;
    }
  },
  rotate: function (degrees) {
    PDFViewerApplication.rotatePages(degrees);
    setTimeout(() => {
      if (window.parent) {
        window.parent.postMessage(
          {
  ...
  getDocumentInfo: function () {
    ...
    return {
      title: PDFViewerApplication.documentInfo?.Title || "",
      author: PDFViewerApplication.documentInfo?.Author || "",
      subject: PDFViewerApplication.documentInfo?.Subject || "",
      keywords: PDFViewerApplication.documentInfo?.Keywords || "",
      creator: PDFViewerApplication.documentInfo?.Creator || "",
      producer: PDFViewerApplication.documentInfo?.Producer || "",
      version: PDFViewerApplication.documentInfo?.PDFFormatVersion || "",
      pageCount: PDFViewerApplication.pagesCount || 0,
      currentPage: PDFViewerApplication.page || 0,
  ...
  saveAnnotations: async function () {
    ...
    try {
      if (PDFViewerApplication.pdfDocument.annotationStorage.size === 0) {
        console.log("No annotations to save");
        return false;
      }
      await PDFViewerApplication.save();
      if (window.parent) {
        window.parent.postMessage(
          {
  ...
  hasUnsavedAnnotations: function () {
    if (
      !PDFViewerApplication.pdfDocument ||
      !PDFViewerApplication.pdfDocument.annotationStorage
    ) {
    ...
    return PDFViewerApplication.pdfDocument.annotationStorage.size > 0;
  },
  clearAnnotations: function () {
    ...
    try {
      PDFViewerApplication.pdfDocument.annotationStorage.clear();
  ...
  on: function (eventName, callback) {
    ...
    console.log(`[PDFViewerAPI] Adding listener for event: ${eventName}`);
    PDFViewerApplication.eventBus._on(eventName, callback);
  },
  off: function (eventName, callback) {
    ...
    PDFViewerApplication.eventBus._off(eventName, callback);
  },
  enableIOSZoom: function () {
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
      ...
      if (PDFViewerApplication._touchManager) {
        const origCallback = PDFViewerApplication.touchPinchCallback;
        PDFViewerApplication.touchPinchCallback = function (
          origin,
          prevDistance,
          distance
        ) {
...
// PDF Selection Popup functionality
// (function () {
//   // Add the feature to the PDFViewerAPI
//   window.PDFViewerAPI = window.PDFViewerAPI || {};

//   // Create the popup feature
//   window.PDFViewerAPI.enableTextSelectionPopup = function (options = {}) {
//     const defaultOptions = {
//       copyEnabled: true,
//       askEnabled: true,
//       flashcardEnabled: true, // Add flashcard option
//       popupDelay: 150, // Milliseconds delay before showing popup
//       popupOffset: 10, // Pixels to offset popup from cursor
//       popupZIndex: 1000, // Z-index of the popup
//       popupClass: "pdf-selection-popup",
//       onAsk: null, // Callback function when Ask is clicked
//       onFlashcard: null, // Callback function when Generate Flashcard is clicked
//     };

//     const settings = { ...defaultOptions, ...options };

//     // Create popup element if it doesn't exist
//     let popup = document.getElementById("pdfSelectionPopup");
//     if (!popup) {
//       popup = document.createElement("div");
//       popup.id = "pdfSelectionPopup";
//       popup.className = settings.popupClass;
//       popup.style.position = "absolute";
//       popup.style.zIndex = settings.popupZIndex;
...
//       // Create SVG icons for buttons
//       if (settings.copyEnabled) {
//         const copyButton = document.createElement("button");
//         copyButton.id = "textcopybtn";
//         copyButton.title = "Copy"; // Basic tooltip

//         // Create custom tooltip
//         const copyTooltip = document.createElement("div");
//         copyTooltip.className = "pdf-selection-tooltip";
...
//           pointer-events: none;
//           z-index: ${settings.popupZIndex + 1};
...
//           copyButton.style.background = "#eaeaea";
//           copyButton.style.transform = "translateY(-2px)";
//           copyButton.style.boxShadow = "0 2px 5px rgba(0,0,0,0.1)";
//           copyButton.querySelector(".pdf-selection-tooltip").style.opacity =
//             "1";
//         });
...
//         askButton.id = "askbtn";
//         askButton.title = "Ask acolyte";

//         // Create custom tooltip
//         const askTooltip = document.createElement("div");
//         askTooltip.className = "pdf-selection-tooltip";
//         askTooltip.textContent = "Ask acolyte";
//         askTooltip.style.cssText = `
//           position: absolute;
//           background: rgba(0,0,0,0.75);
//           color: white;
//           padding: 5px 8px;
...
//           font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
//         `;
//         askButton.appendChild(askTooltip);
//         askButton.style.cssText = buttonStyle;
//         askButton.style.position = "relative";
//         askButton.innerHTML = `
//        <span>Ask Acolyte</span>
//         `;
//         askButton.addEventListener("click", handleAsk);
//         askButton.addEventListener("mouseover", () => {
//           askButton.style.background = "#eaeaea";
//           askButton.style.transform = "translateY(-2px)";
//           askButton.style.boxShadow = "0 2px 5px rgba(0,0,0,0.1)";
//           askButton.querySelector(".pdf-selection-tooltip").style.opacity = "1";
//         });
//         askButton.addEventListener("mouseout", () => {
//           askButton.style.background = "#f8f8f8";
//           askButton.style.transform = "translateY(0)";
//           askButton.style.boxShadow = "none";
//           askButton.querySelector(".pdf-selection-tooltip").style.opacity = "0";
//         });
//         popup.appendChild(askButton);
//       }

//       // Add flashcard button
//       if (settings.flashcardEnabled) {
//         const flashcardButton = document.createElement("button");
//         flashcardButton.id = "flashcardbtn";
//         flashcardButton.title = "Generate Flashcard";

//         // Create custom tooltip
//         const flashcardTooltip = document.createElement("div");
//         flashcardTooltip.className = "pdf-selection-tooltip";
//         flashcardTooltip.textContent = "Generate Flashcard";
//         flashcardTooltip.style.cssText = `
//           position: absolute;
//           background: rgba(0,0,0,0.75);
//           color: white;
//           padding: 5px 8px;
...
//           font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
//         `;
//         flashcardButton.appendChild(flashcardTooltip);
//         flashcardButton.style.cssText = buttonStyle;
//         flashcardButton.style.position = "relative";
//         flashcardButton.innerHTML = `
//         <span>Generate Flashcard</span>
//        `;
//         flashcardButton.addEventListener("click", handleFlashcard);
//         flashcardButton.addEventListener("mouseover", () => {
//           flashcardButton.style.background = "#eaeaea";
//           flashcardButton.style.transform = "translateY(-2px)";
//           flashcardButton.style.boxShadow = "0 2px 5px rgba(0,0,0,0.1)";
//           flashcardButton.querySelector(
//             ".pdf-selection-tooltip"
//           ).style.opacity = "1";
//         });
//         flashcardButton.addEventListener("mouseout", () => {
//           flashcardButton.style.background = "#f8f8f8";
//           flashcardButton.style.transform = "translateY(0)";
//           flashcardButton.style.boxShadow = "none";
//           flashcardButton.querySelector(
//             ".pdf-selection-tooltip"
//           ).style.opacity = "0";
//         });
//         popup.appendChild(flashcardButton);
//       }
//     }
...
//     // Main event listener for selections
//     const viewerContainer = document.getElementById("viewerContainer");
//     if (!viewerContainer) {
//       console.warn(
//         "Viewer container not found, waiting for it to be created..."
//       );

//       // Set a mutation observer to wait for the viewer container to be created
//       const observer = new MutationObserver(function (mutations, obs) {
//         const container = document.getElementById("viewerContainer");
//         if (container) {
//           console.log(
//             "Viewer container now available, setting up selection popup"
//           );
//           obs.disconnect();
...
//       viewerContainer.addEventListener("touchend", handleTextSelection);
//     }

//     // Add click listener to document to hide popup when clicking elsewhere
//     document.addEventListener("click", function (e) {
//       if (e.target.id !== "pdfSelectionPopup" && !popup.contains(e.target)) {
//         popup.style.display = "none";
//       }
//     });

//     // Handle scroll and zoom events to reposition the popup
//     if (viewerContainer) {
//       viewerContainer.addEventListener("scroll", updatePopupPosition);
//       window.addEventListener("resize", updatePopupPosition);
//     }

//     // Listen for PDF.js zoom events
//     if (
//       typeof PDFViewerApplication !== "undefined" &&
//       PDFViewerApplication.eventBus
//     ) {
//       PDFViewerApplication.eventBus._on("scalechanged", updatePopupPosition);
//       PDFViewerApplication.eventBus._on("pagerendered", updatePopupPosition);
//       PDFViewerApplication.eventBus._on("updateviewarea", updatePopupPosition);
//     }

//     // Store the current selection range for repositioning
//     let lastRange = null;

//     // Function to update popup position when scrolling or zooming
//     function updatePopupPosition() {
//       // Only reposition if popup is visible and we have a selection
//       if (popup.style.display !== "none" && lastRange && currentSelection) {
//         try {
//           // Get updated position of the selection
...
//             window.scrollY -
//             popup.offsetHeight -
//             settings.popupOffset
//           }px`;

//           // If popup would be off-screen at the top, position it below
//           if (rect.top - popup.offsetHeight - settings.popupOffset < 0) {
//             popup.style.top = `${
//               rect.bottom + window.scrollY + settings.popupOffset
//             }px`;
//           }
...
//           if (rect.left + popup.offsetWidth > viewportWidth) {
//             popup.style.left = `${viewportWidth - popup.offsetWidth - 10}px`;
//           }

//           // Hide popup if selection is no longer visible in the viewport
//           if (
...
//           popup.style.display = "none";
//           console.log("Error repositioning popup:", e);
//         }
//       }
//     }
...
//           try {
//             // Store reference to the range for later repositioning
...
//               "position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: white; padding: 8px 16px; border-radius: 4px; z-index: 2000; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;";
//             document.body.appendChild(copiedToast);

//             setTimeout(() => {
//               copiedToast.style.opacity = "0";
//               copiedToast.style.transition = "opacity 0.3s ease";
//               setTimeout(() => {
...
//         // Either call the callback or post message to parent
//         if (typeof settings.onAsk === "function") {
//           settings.onAsk(currentSelection);
...
//         // Either call the callback or post message to parent
//         if (typeof settings.onFlashcard === "function") {
//           settings.onFlashcard(currentSelection);
//         } else if (window.parent) {
//           window.parent.postMessage(
//             {
...
//         const flashcardToast = document.createElement("div");
//         flashcardToast.textContent = "Flashcard generated!";
//         flashcardToast.style.cssText =
//           "position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); color: white; padding: 8px 16px; border-radius: 4px; z-index: 2000; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;";
//         document.body.appendChild(flashcardToast);

//         setTimeout(() => {
//           flashcardToast.style.opacity = "0";
//           flashcardToast.style.transition = "opacity 0.3s ease";
//           setTimeout(() => {
//             document.body.removeChild(flashcardToast);
//           }, 300);
//         }, 1200);
//       }
//     }

//     // Add method to disable the popup
//     this.disableTextSelectionPopup = function () {
//       if (viewerContainer) {
//         viewerContainer.removeEventListener("mouseup", handleTextSelection);
//         viewerContainer.removeEventListener("touchend", handleTextSelection);
//         viewerContainer.removeEventListener("scroll", updatePopupPosition);
//       }

//       window.removeEventListener("resize", updatePopupPosition);

//       // Remove PDF.js specific event listeners
//       if (
//         typeof PDFViewerApplication !== "undefined" &&
//         PDFViewerApplication.eventBus
//       ) {
//         PDFViewerApplication.eventBus._off("scalechanged", updatePopupPosition);
//         PDFViewerApplication.eventBus._off("pagerendered", updatePopupPosition);
//         PDFViewerApplication.eventBus._off(
//           "updateviewarea",
//           updatePopupPosition
//         );
//       }
...
//   // Auto-initialize the selection popup
//   function initSelectionPopup() {
//     console.log("[PDF Selection Popup] Initializing text selection popup...");
//     window.PDFViewerAPI.enableTextSelectionPopup();
//   }

//   // Multiple initialization methods to ensure it works in various PDF.js setups

//   // Method 1: Listen for PDFViewerApplication to be ready
//   if (typeof PDFViewerApplication !== "undefined") {
//     if (PDFViewerApplication.initializedPromise) {
//       PDFViewerApplication.initializedPromise.then(() => {
//         console.log(
//           "[PDF Selection Popup] PDFViewerApplication is initialized"
//         );
//         initSelectionPopup();
//       });
//     } else if (PDFViewerApplication.initialized) {
//       console.log(
//         "[PDF Selection Popup] PDFViewerApplication is already initialized"
//       );
//       initSelectionPopup();
//     }
//   }

//   // Method 2: Listen for PDF document loaded event
//   document.addEventListener("documentloaded", function () {
//     console.log("[PDF Selection Popup] Document loaded event detected");
//     initSelectionPopup();
//   });

//   // Method 3: Listen for webviewerloaded event
//   document.addEventListener("webviewerloaded", function () {
//     console.log("[PDF Selection Popup] Webviewer loaded event detected");
//     // Add a small delay to ensure PDFViewerApplication is fully initialized
//     setTimeout(initSelectionPopup, 500);
//   });

//   // Method 4: Fallback with window.load + timeout
//   window.addEventListener("load", function () {
//     setTimeout(function () {
//       if (typeof PDFViewerApplication !== "undefined") {
//         console.log("[PDF Selection Popup] Initializing on window load");
//         initSelectionPopup();
//       } else {
//         // Last resort: check every second until PDFViewerApplication becomes available
//         const checkInterval = setInterval(function () {
//           if (typeof PDFViewerApplication !== "undefined") {
//             console.log("[PDF Selection Popup] PDFViewerApplication detected");
...
// (function disableNativeTouchPopup() {
//   // Prevent context menu on long press
//   document.addEventListener('contextmenu', (e) => {
//     e.preventDefault();
//   });

//   // Prevent long press (touch-and-hold) from triggering the copy/share UI
//   document.addEventListener('touchstart', (e) => {
//     if (e.target.closest('[data-custom-popup]')) {
//       e.preventDefault(); // Prevent default behavior on long touch
//     }
//   }, { passive: false });

//   // Also block touchend to avoid potential triggers on iOS
...
//   // Apply styles to suppress callout and prevent text selection
//   const style = document.createElement('style');
//   style.textContent = `
//     [data-custom-popup] {
//       -webkit-touch-callout: none; /* Disables callout (copy/share) on iOS Safari */
...
///Completely disable all zooming on iOS devices

setTimeout(function () {
  console.log("[PDFViewerAPI] Checking for iOS/iPadOS device...");

  // Thorough iOS detection
  const userAgent = navigator.userAgent;
  const hasIOS_UA = /iPad|iPhone|iPod/.test(userAgent);
  const supportsTouch =
  ...
  const isIOS = hasIOS_UA || isIPadOS;

  if (isIOS) {
    console.log(
      "[PDFViewerAPI] iOS/iPadOS device detected, disabling all zooming"
    );

    // 1. Disable page-level zooming via viewport meta tag
    let viewportMeta = document.querySelector('meta[name="viewport"]');
    if (viewportMeta) {
      viewportMeta.setAttribute(
        "content",
    ...
    // 2. Block all multi-touch and gesture events at document level
    document.addEventListener(
      "touchmove",
      function (e) {
    ...
    // 3. If PDF viewer is available, intercept its zoom methods
    if (window.PDFViewerAPI) {
      console.log("[PDFViewerAPI] Disabling PDF viewer zoom functionality");

      // Store original methods
      const originalZoomIn = window.PDFViewerAPI.zoomIn;
      const originalZoomOut = window.PDFViewerAPI.zoomOut;
      const originalSetZoom = window.PDFViewerAPI.setZoom;

      // Override zoom methods to do nothing on iOS
      window.PDFViewerAPI.zoomIn = function () {
        console.log("[PDFViewerAPI] Zoom in prevented on iOS");
        return false;
      };

      window.PDFViewerAPI.zoomOut = function () {
        console.log("[PDFViewerAPI] Zoom out prevented on iOS");
        return false;
      };

      window.PDFViewerAPI.setZoom = function (scale) {
        console.log("[PDFViewerAPI] Set zoom prevented on iOS");
        return false;
      };

      // 4. Also disable pinch zoom in the viewer container
      const viewerContainer = document.getElementById("viewerContainer");
      if (viewerContainer) {
      ...
      // 5. Try to intercept PDF.js internal zoom events
      if (window.PDFViewerApplication) {
        try {
          // Override PDF.js internal zoom methods if available
          const originalPDFZoomIn = PDFViewerApplication.zoomIn;
          const originalPDFZoomOut = PDFViewerApplication.zoomOut;

          PDFViewerApplication.zoomIn = function () {
            console.log("[PDFViewerAPI] PDF.js zoom in prevented on iOS");
            return;
          };

          PDFViewerApplication.zoomOut = function () {
            console.log("[PDFViewerAPI] PDF.js zoom out prevented on iOS");
            return;
          };

          if (PDFViewerApplication.pdfViewer) {
            // Lock the scale value
            const currentScale =
            ...
            Object.defineProperty(
              ...
              "currentScale",
              {
                ...
                set: function (newValue) {
                  console.log(
                    "[PDFViewerAPI] Preventing scale change from",
                    currentScale,
                    "to",
                    newValue
                  );
                  // Do nothing - scale remains locked
                  return currentScale;
                },
              }
            );
          ...
          console.error("[PDFViewerAPI] Error overriding PDF.js zoom:", e);
        }
      }
    }

    console.log("[PDFViewerAPI] All zoom functionality disabled on iOS/iPadOS");
  }
}, 1000);
...
window.addEventListener("message", function (event) {
  ...
  console.log(`[PDFViewerAPI] Received message from parent:`, type, data);
  try {
    switch (type) {
      case "search":
        console.log("[PDFViewerAPI] Processing search request:", data?.query);
        if (data && data.query) {
          window.PDFViewerAPI.search(data.query, data.options);
        } else {
          window.PDFViewerAPI.clearSearch();
        }
        break;
      case "setAnnotationMode":
        console.log("[PDFViewerAPI] Setting annotation mode to:", data?.mode);
        if (data && data.mode !== undefined) {
          window.PDFViewerAPI.setAnnotationEditorMode(data.mode);
        }
        break;
      case "saveAnnotations":
        console.log("[PDFViewerAPI] Saving annotations");
        window.PDFViewerAPI.saveAnnotations();
        break;
      case "clearAnnotations":
        console.log("[PDFViewerAPI] Clearing annotations");
        window.PDFViewerAPI.clearAnnotations();
        break;
      case "checkStatus":
        console.log("[PDFViewerAPI] Status check requested");
        const status = {
          initialized: !!PDFViewerApplication.initialized,
          documentLoaded: !!PDFViewerApplication.pdfDocument,
          annotationsAvailable:
            !!PDFViewerApplication.pdfViewer?.annotationEditorUIManager,
          currentPage: PDFViewerApplication.page,
          totalPages: PDFViewerApplication.pagesCount,
          currentMode: window.PDFViewerAPI.getAnnotationEditorMode(),
        };
        console.log("[PDFViewerAPI] Current status:", status);
        if (window.parent) {
          window.parent.postMessage(
            {
              type: "statusReport",
        ...
        break;
      default:
        console.log("[PDFViewerAPI] Unknown message type:", type);
    }
  } catch (error) {
    console.error("[PDFViewerAPI] Error processing message:", error);
    if (window.parent) {
      window.parent.postMessage(
        {
          type: "error",
...
window.addEventListener("load", function () {
  console.log("[PDFViewerAPI] Window loaded, initializing...");
  if (window.PDFViewerApplication) {
    console.log("[PDFViewerAPI] PDFViewerApplication found");
  } else {
    console.warn("[PDFViewerAPI] PDFViewerApplication not found!");
  }
  if (pdfjsLib && pdfjsLib.version) {
    console.log("[PDFViewerAPI] PDF.js version:", pdfjsLib.version);
  }
  const checkReadiness = function () {
    if (PDFViewerApplication.initialized && PDFViewerApplication.pdfViewer) {
      console.log("[PDFViewerAPI] PDFViewerApplication initialized");
      console.log("[PDFViewerAPI] Checking for annotation support...");
      if (PDFViewerApplication.pdfViewer.annotationEditorUIManager) {
        console.log("[PDFViewerAPI] Annotation editor UI manager found");
        console.log(
          "[PDFViewerAPI] Available editor types:",
          PDFViewerApplication.pdfViewer.annotationEditorUIManager.editorTypes
        );
      } else {
        console.warn("[PDFViewerAPI] No annotation editor UI manager found!");
      }
      if (PDFViewerApplication.pdfDocument) {
        console.log("[PDFViewerAPI] PDF document loaded");
      } else {
        console.log("[PDFViewerAPI] PDF document not yet loaded");
      }
      console.log("[PDFViewerAPI] User agent:", navigator.userAgent);
      console.log("[PDFViewerAPI] Platform:", navigator.platform);
      if (window.parent && window.parent !== window) {
        console.log(
          "[PDFViewerAPI] Notifying parent window that viewer is ready"
        );
        window.parent.postMessage(
          {
            type: "pdfViewerReady",
      ...
      console.log(
        "[PDFViewerAPI] PDFViewerApplication not fully initialized yet, waiting..."
      );
...
function addSelectionScreenshotTool() {
  // Create button
  const screenshotBtn = document.createElement("button");
  screenshotBtn.textContent = "Select Area Screenshot";
  screenshotBtn.style.position = "fixed";
  screenshotBtn.style.top = "40px";
  screenshotBtn.style.right = "10px";
  screenshotBtn.style.zIndex = "9999";
  screenshotBtn.style.padding = "8px 12px";
  screenshotBtn.style.backgroundColor = "#0074d9";
  ...
  selectionOverlay.style.border = "2px dashed #0074d9";
  selectionOverlay.style.backgroundColor = "rgba(0, 116, 217, 0.1)";
  selectionOverlay.style.zIndex = "9998";
  selectionOverlay.style.pointerEvents = "none";

  // No result modal - we'll only use animation and notifications
  ...
  async function copyImageToClipboard(dataUrl) {
    try {
      ...
      // Also store the data URL in the clipboard for Excalidraw compatibility
      const clipboardData = {
        url: dataUrl,
        bounds: {
          width: img.width,
      ...
      // Store data in localStorage for Excalidraw to access
      localStorage.setItem(
  ...
  window.addEventListener("message", function (event) {
    console.log(event)
    if (event.data.type === "start-pdf-screenshot") {
      // Parent has requested to start selection mode
        // Start selection mode
        isSelecting = true;
        viewerContainer.style.cursor = "crosshair";
        screenshotBtn.textContent = "Cancel Selection";
        screenshotBtn.style.backgroundColor = "#ff4136";
        // alert("Tap and drag to select an area of the PDF to screenshot");

    } else if (event.data.type === "stop-pdf-screenshot") {
      isSelecting = false;
      selectionOverlay.style.display = "none";
  ...
  screenshotBtn.addEventListener("click", function () {
    ...
    if (isSelecting) {
      ...
      screenshotBtn.textContent = "Cancel Selection";
      screenshotBtn.style.backgroundColor = "#ff4136";
      // alert("Click/tap and drag to select an area of the PDF to screenshot");
    }
  });

  // Add download functionality to main button with double click
  screenshotBtn.addEventListener("dblclick", function () {
    if (screenshotDataUrl) {
      const downloadLink = document.createElement("a");
      downloadLink.href = screenshotDataUrl;
      downloadLink.download = `pdf-selection.png`;
      document.body.appendChild(downloadLink);
      downloadLink.click();
      document.body.removeChild(downloadLink);

      // Show a small floating status message for download
      const downloadStatus = document.createElement("div");
      downloadStatus.textContent = "Screenshot downloaded!";
      ...
      downloadStatus.style.backgroundColor = "rgba(46, 204, 64, 0.9)";
      downloadStatus.style.color = "white";
      downloadStatus.style.padding = "8px 16px";
      downloadStatus.style.borderRadius = "4px";
      downloadStatus.style.zIndex = "10001";
      downloadStatus.style.opacity = "0";
      downloadStatus.style.transition = "opacity 0.3s ease-in-out";
  ...
  // Touch start event - start selection
  ...
  // Touch move event - update selection
  ...
  // Mouse up event - complete selection and take screenshot
  ...
  // Touch end event - complete selection and take screenshot
  document.addEventListener("touchend", function (e) {
    if (!isSelecting || startX === undefined) return;

    // For touchend, we use the last recorded endX/endY from touchmove
    // since touchend doesn't have coordinates
    completeSelection();
  });

  // Touch cancel event - cancel selection
  ...
  // Function to complete the selection process
  function completeSelection() {
    ...
    // If selection is too small, treat as a click/tap and ignore
    ...
    takeSelectionScreenshot(left, top, width, height);
  ...
  // Function to capture selected area with higher quality
  function takeSelectionScreenshot(left, top, width, height) {
    try {
      ...
      if (!pageView || !pageView.canvas) {
        alert("Cannot take screenshot: page canvas not available");
        return;
      }

      // Create a temporary canvas with 2x resolution for better quality
      ...
      // Calculate the scaling factor between screen coordinates and PDF coordinates
      const scaleX = pageView.canvas.width / pageRect.width;
      const scaleY = pageView.canvas.height / pageRect.height;

      // Calculate the source rectangle in canvas coordinates
      const sourceX = (left - pageRect.left) * scaleX;
      const sourceY = (top - pageRect.top) * scaleY;
      const sourceWidth = width * scaleX;
      const sourceHeight = height * scaleY;

      // Draw the selected area to our temporary canvas with scaling for better quality
      ctx.drawImage(
        pageView.canvas,
        sourceX,
        sourceY,
      ...
      showScreenshotPopAnimation(left, top, width, height, screenshotDataUrl);
      ...
      window.parent.postMessage(
        {
          type: "pdf-screenshot-capture",
          data: data,
        },
        "*"
      );

      console.log(`Selection screenshot taken of page ${currentPageNumber}`);
    } catch (error) {
      console.error("Error taking selection screenshot:", error);
      alert("Error taking screenshot: " + error.message);
    }
  }

  // Function to show screenshot pop animation
  function showScreenshotPopAnimation(left, top, width, height, imageUrl) {
    // Create animation container
    const animContainer = document.createElement("div");
    animContainer.style.position = "fixed";
    animContainer.style.left = left + "px";
    animContainer.style.top = top + "px";
    animContainer.style.width = width + "px";
    animContainer.style.height = height + "px";
    animContainer.style.zIndex = "10001";
    animContainer.style.pointerEvents = "none";
    animContainer.style.perspective = "1000px";

    // Create image element
    const animImage = document.createElement("img");
    animImage.src = imageUrl;
    animImage.style.width = "100%";
    animImage.style.height = "100%";
    animImage.style.objectFit = "contain";
    animImage.style.transition =
      "all 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
    animImage.style.boxShadow = "0 10px 25px rgba(0,0,0,0.5)";
    animImage.style.border = "2px solid white";
    animImage.style.opacity = "0";
    animImage.style.transform = "scale(0.8) translateY(0px) rotateX(0deg)";
    ...
    setTimeout(() => {
      ...
      animImage.style.transform = "scale(1.1) translateY(-20px) rotateX(5deg)";

      // Hold
      setTimeout(() => {
        // Start fading out and flying away
        animImage.style.opacity = "0";
        animImage.style.transform =
          "scale(1.2) translateY(-50px) rotateX(10deg)";
...
// Run immediately
addSelectionScreenshotTool();
// zoom diables

// ./web/viewer.js

// Function to enable text selection only on the current page

const pdfjsVersion = "5.1.110";
const pdfjsBuild = "b7437376f";
const AppConstants = {
