...
// Updated NaturalLanguageInput component to trigger the calendar popup
...
>(({ placeholder, onClick, ...props }, ref) => {
  ...
  const handleParse = React.useCallback(
    ...
    [disabled, onTimeChange, onValueChange]
  );
  ...
  return (
    <Input
      ...
      className={cn("px-2 mr-0.5 flex-1 border-none h-8 rounded cursor-pointer", inputBase)}
      {...props}
    />
  );
});

// Modified SmartDatetimeInput to connect the input field to the popup
    ...
    >
      <div className="flex items-center justify-center">
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          ...
          >
            <DateTimeLocalInput disabled={false} />
          ...
          <PopoverContent className="w-auto p-0" sideOffset={8}>
            <div className="flex gap-1">
              <Calendar
                ...
                mode="single"
                selected={value}
                onSelect={(date, selectedDate, m, e) => {
                  if (!date) return;
