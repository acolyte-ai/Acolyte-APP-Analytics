...
const ShapeButton = ({ Icon, isSelected, onClick }) => (
  <button
    className={`w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 dark:hover:bg-gray-500 transition-colors ${isSelected ? "bg-gray-500 " : ""
      }`}
    onClick={onClick}
  >
...
const ShapeSelector = () => {
  ...
  return (
    <div className="relative">
      <div className="bg-white dark:bg-black rounded-full py-1.5 px-4 flex items-center gap-1 w-fit shadow-lg dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700">
        {shapes.map((shape) => (
          <ShapeButton
            key={shape.id}
            Icon={shape.icon}
      ...
      {(menuOpen && selectedShape && selectedShape?.id !== "rectangleSelection") && (
        <div className="absolute -top-20">
          ...
          />
          {/* Pointer */}
          <div className="absolute w-3 h-3 bg-white dark:bg-black rotate-45 -bottom-1.5 left-1/2 transform -translate-x-1/2 border-b border-r border-gray-100 dark:border-gray-700" />
        </div>
      )}
    </div>
  );
