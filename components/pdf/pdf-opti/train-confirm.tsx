...
import { markPdfAsStartedTraining, markPdfAsSynced, markPdfAsTrained } from "@/db/pdf/fileSystem";
...
export const TrainAiNotification = ({ openTrainModal, setopenTrainModal }) => {
  ...
  useEffect(() => {
    const sync = async () => {
      ...
      try {
        ...
        toast.error(error?.message || "unable to train file")
  ...
  const processDocument = async (bucketName, userId, key, subject) => {
    try {
  ...
  const handleConfirm = async () => {
    ...
    if (syncing || !currentDocumentId) {
      ...
      toast.error("Provide a valid Document")
      setisTrainingProgress(false)
      return;
    }
    ...
    try {
      ...
      if (isSynced) {
        ...
        if (!uploadResponse.ok) {
          ...
          toast.error("unable to train file")
          setisTrainingProgress(false)
          setSyncing(false);
          setLoading(false);
          return;
      ...
      const files = fileSystem.filter((item) => item.type === "folder" && item.id === extractId.parentId)[0]
      let folderName

      if (files.parentId !== null) {
        const files = fileSystem.filter((item) => item.type === "folder" && item.id === extractId.parentId)[0].parentId
        const fys = fileSystem.filter((item) => item.type === "folder" && item.id === files)[0]
