...
const DarkToggleButton = ({ type = "global" }: Type) => {
  ...
  }, [theme, type, setisDarkMode, setglobalDarkMode, setisDarkFilter]);
  ...
  return (
    <div className="flex items-center justify-center" id="mode">
      <button
        onClick={toggleDarkMode}
        className={`relative w-16 h-8 rounded-full p-1 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 ${isDark
      ...
      >
        ...
        <div
          className={`absolute inset-0 rounded-full transition-opacity duration-300 ${isDark
        ...
        {/* Toggle indicator */}
        <motion.div
          className={`relative z-10 w-6 h-6 rounded-full shadow-md flex items-center justify-center transition-colors ${isDark ? "bg-indigo-400" : "bg-amber-400"
            }`}
          animate={{
            x: isDark ? 32 : 0,
          }}
