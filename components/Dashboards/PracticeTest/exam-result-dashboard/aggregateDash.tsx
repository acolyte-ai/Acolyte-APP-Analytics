...
import SubjectInvolvementCard from '@/components/Exam/dailytest/test-testInvolvement';
...
const AggregateTestSummary = () => {
    ...
    const init = async () => {
        try {
            ...
            const response = await axios.get(process.env.NEXT_PUBLIC_PM_BASE_URL + "/dev/v1/exam/aggregate/" + id);
        ...
        <div className="h-full  w-full overflow-y-auto no-scrollbar relative">
            ...
            <div className="px-8 py-6 max-md:w-full  max-md:flex  max-md:justify-start md:flex-shrink-0">
                <div className="flex justify-between max-md:w-fit md:w-full items-end max-md:flex-col max-md:justify-start max-md:items-start max-md:gap-2">
                    <div className='flex justify-between max-lg:flex-col max-lg:justify-start max-lg:items-start  w-full items-center'>
                        <h1 className="  leading-[50px] text-[26px] font-causten-semibold tracking-normal  text-[#228367] dark:text-white">
                            {data?.examTopic} Test - Completed
                        </h1>
                        ...
                        <div className='flex items-center gap-5'>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent className='h-52 overflow-hidden overflow-y-auto no-scrollbar w-52' >
                                    <DropdownMenuLabel>Total Attempts {data?.attemptsCount || 0}</DropdownMenuLabel>
                                    <DropdownMenuSeparator />
                                    {
                                        data?.attemptsList.map((item, index) => (
                                            <DropdownMenuItem key={item} onClick={() => router.push("/assesment/dash/" + id + "/" + item)}>
                                                attempt {index + 1}
                                            </DropdownMenuItem>
                                        ))
                                    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 px-8  ">
                ...
                <div className='md:col-span-1 col-span-full  w-full h-[249px]'>
                    <SubjectInvolvementCard data={data} />
                </div>
...
export default AggregateTestSummary;